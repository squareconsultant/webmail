...
*/
  ...
  fb.mousemove = function (event) {
    ...
    //get the hex
    var hex = $('#colorpicker-mirror').val();

    var brightness = detectBrightness(hex);

    //set the newly added swatch to the new color
    $('.blank-swatch input[type="button"]').css('background-color',hex).attr('data-color',hex);

    //let's change the dropet color as well
    $('.droplet-splash').css('background-color',hex);

    //change the focussed input
    $('.colorpicker-value').not('.unfocussed').val(hex).css('background-color',hex).css('color',brightness);

    var colorpicker = $('.colorpicker-value').not('.unfocussed');

    if($('#image-editing-canvas-colorpicker').is(':visible')){

         //get the hex
         var hex = $('.image-editing-color-field').val();

         updateOverlayColor(hex);
...
*/
  ...
  fb.mouseup = function () {
    ...
    //saved colorpicker value
    savedColorpickerValue = $('.colorpicker-value').not('.unfocussed').val();

    //if a blank swatch exists
    if($('.blank-swatch').length > 0){ blankSwatchExists = true; }
